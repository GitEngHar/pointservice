...
func (p PointRepository) GetPointByUserID(ctx context.Context, userID string) (domain.Point, error) {
	...
	var query = `SELECT point_num FROM point_root WHERE user_id=$1`
	// TODO: 取得した値を処理する
	row := p.db.QueryRowContext(ctx, query, userID)
	var pointNum int
	if err := row.Scan(&pointNum); err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			return domain.Point{}, fmt.Errorf("no point record found for user_id=%d: %w", userID, err)
		}
		return domain.Point{}, fmt.Errorf("failed to scan point row: %w", err)
	}
...
func (p PointRepository) UpdatePointByUserID(ctx context.Context, point domain.Point) error {
	var query = `UPDATE FROM point_root SET point=$1 WHERE user_id=$2`
	// TODO: 取得した値を処理する
	_, err := p.db.Exec(query, point.PointNum, point.UserID)
	if err != nil {
		return fmt.Errorf("failed to scan point row: %w", err)
	}
	return nil
}
